...
if ($W3SVC -ne $null -and $W3SVC.Status -eq "Stopped") {
    ...
    $Status_decision = Read-Host "`nWould you like to start the service ? [Yes | No]"
    ...
    if ($Status_decision -eq "Yes") {
        ...
        Write-Host "`n[$W3SVC] found [$status] on IIS-Test1" -ForegroundColor green
    }
...
elseif ($W3SVC -ne $null -and $W3SVC.status -eq 'Running') {
    $status = $W3SVC.status
